...
open class JOEmojiableBtn: UIButton {
    ...
    fileprivate func activate() {
        if !active {
            ...
            UIView.animate(withDuration: 0.2, animations: { () -> Void in
                self.options.frame.origin.y = self.origin.y - (self.config.spaceBetweenComponents + sizeBtn.height)
                self.options.alpha = 1
            })

            for i in 0..<dataset.count {
                let imageFrame = CGRect(x: (CGFloat(i + 1) * config.spacing) + (config.size * CGFloat(i)), y: sizeBtn.height * 1.2, sideSize: 10)
                ...
                UIView.animate(withDuration: 0.2, delay: 0.05 * Double(i), options: .curveEaseInOut, animations: { () -> Void in
                    ...
                    option.alpha = 1
                    option.frame.size = CGSize(sideSize: self.config.size)
            ...
            information = InformationView(frame: CGRect(x: 0, y: origin.y, width: frameSV.width, height: frame.height))
    ...
    fileprivate func loseFocus() {
        ...
        selectedItem = -1
        information.activateInformationView(true)
        
        UIView.animate(withDuration: 0.3) { () -> Void in
            let sizeBtn = CGSize(width: ((CGFloat(dataset.count + 1) * self.config.spacing) + (self.config.size * CGFloat(dataset.count))), height: self.config.size + (2 * self.config.spacing))
            self.options.frame = CGRect(x: self.origin.x, y: self.origin.y - (self.config.spaceBetweenComponents + sizeBtn.height), width: sizeBtn.width, height: sizeBtn.height)
            self.options.layer.cornerRadius = sizeBtn.height / 2
            for (idx, view) in self.options.subviews.enumerated() {
                view.frame = CGRect(x: (CGFloat(idx + 1) * self.config.spacing) + (self.config.size * CGFloat(idx)), y: self.config.spacing, sideSize: self.config.size)
            }
        }
    }
    ...
    func selectIndex(_ index: Int) {
        ...
        if index >= 0 && index < dataset.count {
            selectedItem = index
            information.activateInformationView(false)

            UIView.animate(withDuration: 0.3) { () -> Void in
                let sizeBtn = CGSize(width: ((CGFloat(dataset.count - 1) * self.config.spacing) + (self.config.minSize * CGFloat(dataset.count - 1)) + self.config.maxSize), height: self.config.minSize + (2 * self.config.spacing))
                self.options.frame = CGRect(origin: CGPoint(x: self.origin.x,
                                                            y: self.origin.y - (self.config.spaceBetweenComponents + sizeBtn.height)),
                                            size: sizeBtn)
                self.options.layer.cornerRadius = sizeBtn.height / 2
                ...
                for (idx, view) in self.options.subviews.enumerated() {
                    ...
                    default:
                        view.frame = CGRect(x: last, y: self.config.spacing, sideSize: self.config.minSize)
                        view.center.y = (self.config.minSize / 2) + self.config.spacing
